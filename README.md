...
Every image is built from the repository root so it can see `packages/model`:
    ...
    docker build -f apps/freezebot/Dockerfile .

Image names are unchanged: `freezingsaddles/freezing-web`, `-sync` and `-nq`;
Freezebot's is `freezingsaddles/freezing-freezebot`.

## CI

Pull requests run lint, tests, a Bandit scan and a build of all four images
